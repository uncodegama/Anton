...
@weather.get(
    "/forecasts_current",
    response_model=list[WeatherForecastCurrentModel],
    description="Returns current weather forecasts for all locations in db.",
)
async def get_weather_forecasts_current() -> list[WeatherForecastCurrent]:
    query = perform_db_operation(DATABASE_SELECT_TABLE_WEATHER_CURRENTS, ())

    if not query:
        raise HTTPException(status_code=404, detail="Forecast not found.")
    return [WeatherForecastCurrent(*weather) for weather in query]
...
@weather.get(
    ...
    description="Returns all weather forecast (current, hourly, daily) for given id.",
)
